...
export default class SearchArea extends React.Component {
  ...
  selectCondition = (val, e) => {
    ...
    const isAreadyHasItem = selected_conditions.find(
  ...
  renderMatchConditions = () => {
    ...
    const isChooseItem = ({ value, id }) => {
      //是否已选
      const flag = selected_conditions.find(
        item => item.value == value && item.id == id
      )
      return !!flag
    }
          ...
          <div className={indexstyles.match_conditions_item_detail}>
            {conditions.map((val2, key2) => {
              ...
              return (
                ...
                >
                  <div
                    className={`${globalStyles.authTheme} ${globalStyles.global_ellipsis} ${indexstyles.match_conditions_item_detail_item_name}`}
                  >
                    {name}
                  </div>
                  {isChooseItem({ value, id }) && (
                    <div
                      className={`${globalStyles.authTheme} ${indexstyles.match_conditions_item_detail_item_check}`}
                    >
                      &#xe7fc;
                    </div>
                  )}
        ...
        <InputGroup compact={true} size={'large'} style={{ display: 'flex' }}>
          ...
          >
            {searchTypeList.map((value, key) => {
              ...
              return (
                <Option value={search_type} key={key}>
                  {name || ''}
                </Option>
              )
            })}
          ...
          {/*/>*/}
          <Input
            style={{ width: '84%', fontSize: 14, marginLeft: '-10px' }}
            value={searchInputValue}
            ...
            suffix={
              <div
                ...
                onClick={this.getGlobalSearchResult}
              >
                &#xe611;
              </div>
            }
      ...
      <div className={indexstyles.fixed_conditions_area}>
        <div className={indexstyles.fixed_conditions_area_left}>
          {fixed_conditions.map(val => {
            ...
            return (
              <div
                className={indexstyles.fixed_conditions_area_left_item}
                key={`${id}_${name}`}
                onClick={() => this.fixedConditionSelect(query_conditions)}
              >
                {name}
              </div>
            )
