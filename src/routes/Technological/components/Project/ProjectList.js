...
export default class Projectlist extends React.Component {
  ...
  render() {
    ...
    const { current_org = {} } = localStorage.getItem('userInfo')
    ...
    const addItem = (
      <div className={indexStyle.addListItem} onClick={this.addItem.bind(this)}>
        <Icon
          type="plus-circle-o"
          style={{ fontSize: 18, color: '#8c8c8c', marginTop: 6 }}
        />
      </div>
    )
    return (
      <div className={indexStyle.projectListOut}>
        <Collapse
          ...
          activeKey={collapseActiveKeyArray}
        >
          <Panel
            header={`我收藏的${currentNounPlanFilterName(PROJECTS)}`}
            key="1"
          ...
          >
            {star.map((value, key) => {
              ...
              return (
                <ElseProject
                  {...this.props}
                  itemDetailInfo={value}
                  key={`${board_id}_${is_star}`}
                />
              )
            })}
            {/*{addItem}*/}
          ...
          {identity_type == '1' ? (
            <Panel
              header={`我管理的${currentNounPlanFilterName(PROJECTS)}`}
              key="2"
              style={customPanelStyle}
            >
          ...
          <Panel
            header={`我参与的${currentNounPlanFilterName(PROJECTS)}`}
            key="3"
            style={customPanelStyle}
          >
