...
export default class ItemTwoChirldren extends React.Component {
  ...
  itemOneClick() {
    ...
    const {
      ItemTwoChirldrenVaue,
      ItemTwoChirldrenIndex,
      taskGroupListIndex,
    ...
    new_arr_[taskGroupListIndex]['card_data'][taskGroupListIndex_index][
      'child_data'
    ][ItemTwoChirldrenIndex]['is_realize'] = is_realize === '1' ? '0' : '1'
  ...
  render() {
    const {
      ItemTwoChirldrenVaue: { card_name, card_id, is_realize = '0' }
    } = this.props
    return (
      <div key={'1'} className={CreateTaskStyle.item_2_chirld}>
        <div
          ...
          onClick={this.itemOneClick.bind(this, card_id)}
        >
          <Icon
            type="check"
            style={{
              color: is_realize === '1' ? '#FFFFFF' : '#F5F5F5',
              fontSize: 12,
              fontWeight: 'bold'
            }}
          />
