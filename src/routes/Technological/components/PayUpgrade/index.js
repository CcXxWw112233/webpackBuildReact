...
class PayUpgrade extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <div className={indexStyles.infoTitle}>
          协作人数与项目数统计
          {paymentInfo.is_free_trial && paymentInfo.is_free_trial == '0' && (
            <span style={{ color: '#1890FF' }}>(已付费组织)</span>
          )}
        </div>
          ...
          <div className={indexStyles.organizationInfoWrapper}>
            <div className={indexStyles.organizationInfo}>
              <div className={indexStyles.organizationInfoRow}>
                <div className={indexStyles.leftInfo}>协作人数</div>
                <div className={indexStyles.centerInfo}>
                  <Progress
                    strokeWidth={16}
                    strokeColor={'#FFC069'}
                    percent={
                      (infoData.person.currentNum / infoData.person.limit) * 100
                    }
                    showInfo={false}
                  />
                </div>
              ...
              <div className={indexStyles.organizationInfoRow}>
                <div className={indexStyles.leftInfo}>项目数</div>
                <div className={indexStyles.centerInfo}>
                  <Progress
                    strokeWidth={16}
                    strokeColor={'#69C0FF'}
                ...
                <div className={indexStyles.rightInfo}>
                  {infoData.project.currentNum}/{' '}
                  {paymentInfo.is_free_trial == 0 &&
                  infoData.project.limit == 99999
                    ? '不限'
                    : infoData.project.limit}
                </div>
              </div>
            </div>
            ...
            {this.itemList.map((item, index) => {
              return (
                <div key={index} className={indexStyles.item}>
                  ...
                  <div className={indexStyles.itemTitle}>{item.name}</div>
                </div>
              )
            })}
          </div>
