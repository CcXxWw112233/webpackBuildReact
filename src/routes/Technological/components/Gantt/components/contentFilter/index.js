...
export default class ContentFilter extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!dropdownVisible) {
      //当触发关闭的时候，保存下当前选中的值，作为button disabled的比较
      this.setState({
        local_group_view_filter_boards: group_view_filter_boards,
        local_group_view_filter_users: group_view_filter_users
      })
  ...
  renderTreeSelect = ({ treeType }) => {
    ...
    const {
      group_view_users_tree = [],
      group_view_boards_tree = []
    } = this.props
    ...
    const value =
      treeType == 'boards'
        ...
        : local_group_view_filter_users
    const treeDatas =
      treeType == 'boards' ? group_view_boards_tree : group_view_users_tree
    return (
      <TreeSelect
        treeData={treeDatas}
        value={value}
    ...
    return (
      <div className={indexStyles.filter_item}>
        <div className={indexStyles.filter_item_top}>
          ...
          <div
            className={`${indexStyles.filter_item_reset} ${globalStyles.link_mouse}`}
            onClick={() => this.resetTreeSelected({ treeType })}
          >
            重置
          </div>
        </div>
  ...
  Button_disabled = () => {
    ...
    return (
      this.equar(local_group_view_filter_boards, group_view_filter_boards) &&
  ...
  render() {
    return (
      <div
        className={`${indexStyles.content_filter_out} ${globalStyles.global_card}`}
      >
        <div className={`${indexStyles.content_filter_top}`}>
          <div className={`${indexStyles.content_filter_title}`}>内容过滤</div>
          <div
            className={`${globalStyles.authTheme} ${indexStyles.content_filter_close}`}
        ...
        <div className={`${indexStyles.content_filter_footer}`}>
          <Button
            ...
            className={`${!this.Button_disabled() &&
              indexStyles.button_nomal_background} ${indexStyles.check_button}`}
          >
            确定
          </Button>
        </div>
