...
export default class ProccessContent extends React.Component {
  ...
  initCanvas() {
    const {
      datas: { processInfo = {}, processEditDatas = [] }
    } = this.props.model
  ...
  render() {
    ...
    const {
      ...
      privileges = [],
      is_privilege
    } = processInfo //status 1 正在进行 2,暂停 3完成
    const data =
      this.props.model.datas &&
      this.props.model.datas.projectDetailInfoData &&
      this.props.model.datas.projectDetailInfoData.data
        ? this.props.model.datas.projectDetailInfoData.data
        : []
    ...
    return (
      <div>
        ...
        {parseInt(this.props.model.datas.processCurrentCompleteStep) ===
        parseInt(this.props.model.datas.processInfo.node_amount) ? (
          <span
        ...
        >
          {this.props.model.datas.processCurrentCompleteStep
            ? this.props.model.datas.processCurrentCompleteStep
            : 1}
        ...
        >
          ...
          >
            {this.props.model.datas.processInfo.description
              ? delHtmlTag(this.props.model.datas.processInfo.description)
              : '暂无描述'}
          </div>
        </div>
        {/* 这里将会有一个蒙层 */}
