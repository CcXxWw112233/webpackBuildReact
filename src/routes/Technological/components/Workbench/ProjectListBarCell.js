...
const ProjectListBarCell = ({
  ...
  const jumpToProject = (board_id, apps = []) => {
    ...
    const getAppKey = apps =>
      apps.find(item => item.unique_key === '3') ? '3' : apps[0]['unique_key']
    const url = `/#/technological/projectDetail?board_id=${board_id}&appsSelectKey=${getAppKey(
      apps
    )}`
  ...
  org_name =
    is_show_org_name && getOrgNameWithOrgIdFilter(org_id, currentUserOrganizes)
  ...
  return (
    ...
    >
      <Tooltip
        title={org_name ? board_name + ' #' + org_name : board_name}
        placement="topLeft"
      >
      ...
      <Tooltip title="进入项目">
        <span
          className={styles.projectListBarCellInterProject}
          onClick={e => handleJumpToProject(e, board_id, apps, org_id)}
        >
