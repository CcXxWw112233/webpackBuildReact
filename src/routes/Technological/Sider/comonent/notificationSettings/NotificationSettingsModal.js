...
export default class NotificationSettingsModal extends Component {
    ...
    getUsersNoticeSettingList().then(res => {
      if (isApiResponseOk(res)) {
        ...
        this.setState({
          ...
          user_detail_setting_options:
            res.data.notice_model == '1' ? res.data.notice_list_ids : [],
          user_simple_setting_options:
            res.data.notice_model == '2' ? res.data.notice_list_ids : [],
  ...
  hideOkModal = () => {
    ...
    const data = {
      notice_way_data: is_way_status,
      notice_model: radio_checked_val,
      notice_list_ids: new_notice_list_ids,
      notice_daily: notice_daily
    }
  ...
  chgDisplayBlock = optionArr => {
    ...
    if (radio_checked_val == '1') {
      temp_options =
        notice_model == '1' && !is_click_recover
          ? compare_options
          : default_copy_options['detailed']
    } else if (radio_checked_val == '2') {
      temp_options =
        notice_model == '2' && !is_click_recover
          ? compare_options
          : default_copy_options['briefly']
    }
  ...
  renderSetOptions() {
    ...
    let new_detail_default_options =
      notice_model == '1'
        ? user_detail_setting_options && [...user_detail_setting_options]
        : default_detail_options // 将数据中的列表重新解构出来进行数据操作 默认选中的选项
    let new_simple_default_options =
      notice_model == '2'
        ? user_simple_setting_options && [...user_simple_setting_options]
        : default_simple_options
    ...
    const settingContain = (
      <div className={styles.wrapper}>
        <div className={styles.top}>
          ...
          <div className={styles.checkbox}>
            <Checkbox.Group style={{ width: '100%' }} value={status_val}>
              <Row>
                {notice_way_data &&
                  notice_way_data.map(item => {
                    return (
                      <Col span={8} style={{ marginBottom: '8px' }}>
                        <Checkbox
                          disabled={item.disabled}
                          onChange={this.chgMovemenet}
                          value={item.key}
                        >
                          {item.name}
                        </Checkbox>
        ...
        <div className={styles.contant}>
            ...
            >
              ...
              <Radio value="2">
                ...
                <span style={{ display: is_simple_none }}>
                  ...
                  <span
                    onClick={e => {
                      this.handleRecover(e, 2)
                    }}
                    className={styles.simple_recover}
                  >
                    恢复默认设置
                  </span>
                  )
          ...
          <div
            className={` ${glabalStyles.global_vertical_scrollbar} ${styles.set_options}`}
          >
