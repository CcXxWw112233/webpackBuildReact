    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        const {
          ...
          form_id,
          form_data
        } = this.props //isFillForm form_id填写表单的特殊处理
        const {
          datas: { processInfo = {} }
        } = this.props.model
        ...
        if (operateType === '1') {
          if (isFillForm) {
            ...
            this.props.fillFormComplete
              ? this.props.fillFormComplete(obj)
              : false
          } else {
            this.props.completeProcessTask
              ? this.props.completeProcessTask(values)
              : false
            this.props.dispatch({
          ...
          this.props.rebackProcessTask
            ? this.props.rebackProcessTask(values)
            : false
        } else if (operateType === '2') {
          this.props.rejectProcessTask
            ? this.props.rejectProcessTask(values)
            : false
        }
      }
    })
    ...
    const step_3 = (
      ...
      >
        ...
        <FormItem>
          <Button
            type="primary"
            disabled={enableOpinion === '1' ? stepContinueDisabled : false}
            htmlType={'submit'}
