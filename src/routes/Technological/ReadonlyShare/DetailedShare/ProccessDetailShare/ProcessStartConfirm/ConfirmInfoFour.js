...
export default class ConfirmInfoFour extends React.Component {
  ...
  componentWillMount(nextProps) {
    ...
    const { assignees, assignee_type, recipients, cc_type } = processEditDatas[
      itemKey
    ]
    //推进人来源
    let usersArray = []
            ...
            <div>
              <Dropdown
                overlay={
                  <MenuSearchMultiple
                    usersArray={users}
                    setAssignees={this.setRecipients.bind(this)}
                  />
                }
              >
                    ...
                    {recipientsArray.map((value, key) => {
                      if (key <= 20)
                        return (
                          ...
                          >
                            <div>
                              {imgOrAvatar2({ users: users, user_id: value })}
                            </div>
                          </Tooltip>
                        )
                    })}
                    {recipientsArray.length > 20 ? (
                      <span
                        style={{ color: '#595959' }}
                      >{`等${recipientsArray.length}人`}</span>
                    ) : (
                      ''
                    )}
                  </div>
                ) : (
                  <div style={{ color: '#1890FF' }}>设置抄送人</div>
                )}
              </Dropdown>
            </div>
          )
