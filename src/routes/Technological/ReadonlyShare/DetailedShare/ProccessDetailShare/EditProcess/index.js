...
import {
  ...
  processEditDatasRecordsItemOneConstant
} from '../constant'
import {
...
export default class EditProcess extends React.Component {
  ...
  nodeTypeClick(node_type) {
    ...
    if (processCurrentEditStep === 0 && node_type === '5') {
      message.warn(
        `${currentNounPlanFilterName(FLOWS)}节点第一步不能为审批类型`
      )
      return false
    }
    const alltypedata =
    ...
    for (let val of processEditDatasRecords[processCurrentEditStep][
      'alltypedata'
    ]) {
  ...
  verrificationForm(processEditDatas) {
    //校验表单
    ...
    if (currentData['node_type'] === '4' && currentData['cc_type'] === '2') {
      //抄送
  ...
  addNode(node_type) {
    ...
    const recordItemobjs = JSON.parse(
      JSON.stringify(processEditDatasRecordsItemOneConstant)
    )
  ...
  requestFilterProcessEditDatas() {
    ...
    for (let i = 0; i < newProcessEditDatas.length; i++) {
      ...
      if (
        newProcessEditDatas[i]['node_type'] === '4' &&
        newProcessEditDatas[i]['cc_type'] === '1'
      ) {
        //抄送
  ...
  deleteProcessStep(e) {
    ...
    if (processEditDatasRecords.length) {
      ...
      newProcessEditDatasRecords = JSON.parse(
        JSON.stringify(processEditDatasRecords)
      )
  ...
  render() {
    ...
    const filterForm = node_type => {
      ...
      switch (node_type) {
        case '1':
          containner = <EditFormOne />
          break
        case '2':
          containner = <EditFormTwo />
          break
        case '3':
          containner = <EditFormThree />
          break
        case '4':
          containner = <EditFormFour />
          break
        case '5':
          containner = <EditFormFive />
          break
        default:
          containner = ''
          break
    ...
    return (
      <div className={indexStyles.editProcessOut}>
        <div className={indexStyles.editProcessLeft}>
          ...
          {processEditDatas.map((value, key) => {
            return (
              <div
                ...
                className={
                  processCurrentEditStep === key
                    ? indexStyles.itemSelect
                    : indexStyles.item
                }
                onClick={this.currentEditStepClick.bind(this, { value, key })}
              >
                ...
                {processCurrentEditStep === key && key != 0 ? (
                  <div
                    className={`${globalStyles.authTheme} ${indexStyles.itemDelete}`}
                    onClick={this.deleteProcessStep.bind(this)}
                  >
                    &#xe70f;
                  </div>
                ) : (
          ...
          <div
            className={indexStyles.addItem}
            onClick={this.addNode.bind(this, node_type)}
          >
        ...
        <div className={indexStyles.editProcessMiddle}>
          <div className={indexStyles.title}>
            ...
            <div className={indexStyles.right}>
              <div
                className={node_type === '1' ? indexStyles.selectType : ''}
                onClick={this.nodeTypeClick.bind(this, '1')}
              >
                里程碑
              </div>
              <div
                className={node_type === '2' ? indexStyles.selectType : ''}
                onClick={this.nodeTypeClick.bind(this, '2')}
              >
                上传
              </div>
              <div
                className={node_type === '3' ? indexStyles.selectType : ''}
                onClick={this.nodeTypeClick.bind(this, '3')}
              >
                填写
              </div>
              <div
                className={node_type === '4' ? indexStyles.selectType : ''}
                onClick={this.nodeTypeClick.bind(this, '4')}
              >
                抄送
              </div>
              <div
                className={node_type === '5' ? indexStyles.selectType : ''}
                onClick={this.nodeTypeClick.bind(this, '5')}
              >
                审批
              </div>
            </div>
        ...
        <div className={indexStyles.editProcessRight}>
          ...
          <Button
            type={'primary'}
            style={{ marginTop: 36 }}
            onClick={this.setSaveTemplateModalVisible.bind(this)}
          >
            保存模板
          </Button>
          <Button
            style={{ marginTop: 14 }}
            onClick={this.directStart.bind(this)}
          >
            直接启动
          </Button>
          <Button
            style={{ marginTop: 14, color: 'red' }}
            onClick={this.quitEdit.bind(this)}
          >
            退出编辑
          </Button>
        </div>
        <SaveTemplate
          requestFilterProcessEditDatas={this.requestFilterProcessEditDatas.bind(
            this
          )}
          setSaveTemplateModalVisible={this.setSaveTemplateModalVisible.bind(
            this
          )}
          saveTemplateModalVisible={this.state.saveTemplateModalVisible}
        />
