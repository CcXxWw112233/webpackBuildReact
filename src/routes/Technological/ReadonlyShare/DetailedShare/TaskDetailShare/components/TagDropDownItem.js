    ...
    const selectColorNode = (
      <div className={TagDropDownStyles.selectColorOut}>
        {colorArray.map((value, key) => {
          return (
            <div
              onClick={this.setColor.bind(this, value)}
              key={key}
              style={{
                color: `rgba(${value})`,
                backgroundColor: `rgba(${value},0.1)`,
    ...
    return (
      <div
        ...
        style={{
          ...
          border: `1px solid rgba(${color},${onMouse ? '1' : '0.6'})`
        }}
      >
        ...
        <div
          className={TagDropDownStyles.dropItem_right}
          style={{ display: `${onMouse ? 'flex' : 'none'}` }}
          onClick={this.stopPropagation.bind(this)}
        >
          {id != boardTagList[0]['id'] ? (
            <div
              className={globalStyles.authTheme}
              style={{ display: `${!isInEdit ? 'block' : 'none'}` }}
          ...
          <div
            className={globalStyles.authTheme}
            style={{ display: `${!isInEdit ? 'none' : 'block'}` }}
            onClick={this.toCheckEdit.bind(this)}
          >
            &#xe70b;
          </div>
        </div>
