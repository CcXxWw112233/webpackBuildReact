...
export default class ProjectRole extends React.Component {
  ...
  onCheckAllChange = ({ parentKey, childKey }, e) => {
    const {
      datas: { project_role_data }
    } = this.props.model
    let arr = []
    for (let val of project_role_data[parentKey]['function_tree_data'][
      childKey
    ]['child_data']) {
      arr.push(val.id)
    }
    project_role_data[parentKey]['function_tree_data'][childKey]['selects'] = e
      .target.checked
      ? arr
      : []
    project_role_data[parentKey]['function_tree_data'][childKey][
      'indeterminate'
    ] = false
    project_role_data[parentKey]['function_tree_data'][childKey]['checkedAll'] =
      e.target.checked
  ...
  groupOnChange = ({ parentKey, childKey }, checkedList) => {
    ...
    project_role_data[parentKey]['function_tree_data'][childKey]['checkedAll'] =
      checkedList.length === arr.length
    project_role_data[parentKey]['function_tree_data'][childKey][
      'indeterminate'
    ] = !!checkedList.length && checkedList.length < arr.length
  ...
  finallySave({ value, parentKey }) {
    ...
    const {
      ...
      function_tree_data = [],
      content_tree_data = [],
      already_has_content_permission_trans = []
    } = value
  ...
  render() {
      ...
      return (
        <Menu onClick={this.handleMenuClick.bind(this, { parentKey, value })}>
          ...
          checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_EDIT) ? (
            ...
            >
              <div className={indexStyles.elseProjectMemu}>设为默认</div>
            </Menu.Item>
          ) : (
            ''
          )}
          {checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_CREATE) && (
            <Menu.Item
              ...
              style={{ textAlign: 'center', padding: 0, margin: 0 }}
            >
              <div className={indexStyles.elseProjectMemu}>复制</div>
            </Menu.Item>
          )}
          {checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_EDIT) && (
            <Menu.Item
              ...
              style={{ textAlign: 'center', padding: 0, margin: 0 }}
            >
              <div className={indexStyles.elseProjectMemu}>重命名</div>
            </Menu.Item>
          )}
          {is_default !== '1' &&
          checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_DELETE) ? (
            <Menu.Item
              ...
              style={{ textAlign: 'center', padding: 0, margin: 0 }}
            >
              <div className={indexStyles.elseProjectDangerMenu}>删除</div>
            </Menu.Item>
          ) : (
            ''
          )}
      ...
      return data.map(item => {
        if (item.app_data) {
          return (
            <TreeNode
              key={
                item.app_id ? `${item.board_id}__${item.app_id}` : item.board_id
              }
              title={2}
            >
              {loop(item.app_data)}
    ...
    return (
      <div className={indexStyles.TabPaneContent}>
              ...
              <Panel
                header={
                  <div className={indexStyles.parrentPanaelHeader}>
                    ...
                    <div
                      className={indexStyles.parrentPanaelHeader_r}
                      style={{ display: 'block' }}
                    >
                      {system_role !== '1' && is_visitor !== '1' ? (
                        <Dropdown
                          overlay={operateMenu({ parentKey, value })}
                          trigger={['click']}
                        >
              ...
              >
                ...
                <Collapse bordered={false}>
                  ...
                  {function_tree_data.map((value, childKey) => {
                    const {
                      ...
                      indeterminate,
                      selects
                    } = value //indeterminate, checkedAll
                    const { name } = value
                    return (
                      <Panel
                        header={
                      ...
                      >
                        <div className={indexStyles.childrenPanelContent}>
                          ...
                          <Checkbox.Group
                            style={{ width: '100%' }}
                            onChange={this.groupOnChange.bind(this, {
                              parentKey,
                              childKey
                            })}
                            value={selects}
                ...
                <div style={{ marginTop: 10 }}>
                  ...
                  <Tree
                    checkable
                    multiple
                    onCheck={this.onCheck.bind(this, parentKey)}
                    disabled={checkDisabled}
                    checkedKeys={already_has_content_permission_trans}
                  >
        ...
        {checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_CREATE) && (
          <div
            className={indexStyles.addParrentPanel}
        ...
        {/*重命名,添加*/}
        <RenameModal
          reName_Add_type={this.state.reName_Add_type}
          copy={this.copy.bind(this)}
          renameModalVisable={this.state.renameModalVisable}
          reNamePanelItem={this.reNamePanelItem.bind(this)}
          addPanelItem={this.addPanelItem.bind(this)}
          setRenameModalVisable={this.setRenameModalVisable.bind(this)}
        />
      </div>
    )
  }
...
const childrenPanelStyles = {
  //子panel
  borderBottom: '1px dashed rgba(0,0,0,.2)',
  position: 'relative'
}
const checkBoxAllStyles = {
  //全选checkbox外层div
  position: 'absolute',
  top: 12,
  left: 40
}
const childrenPanelRowsStyles = {
  //子panel下的Row
  lineHeight: '34px'
}
