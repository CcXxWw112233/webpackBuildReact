...
export default class NounDefinition extends React.Component {
  ...
  saveNoun() {
    ...
    const {
      datas: { current_scheme_local, current_scheme_id }
    } = this.props.model
    let values = []
      ...
      <div>
        <div className={indexStyles.nounDefinitout} ref={`myInput`}>
          {scheme_data.map((parentValue, parentKey) => {
            ...
            return (
              <div
                className={`${indexStyles.nounDefinit_col} ${
                  current_scheme_id === id && parentKey !== 0
                    ? indexStyles.selectedDefinite2
                    : ''
                }`}
                key={parentKey}
              >
                ...
                <div
                  className={indexStyles.nounDefinit_col_title}
                  key={parentKey}
                  onClick={this.selectCol.bind(this, {
                    id,
                    name,
                    editable,
                    parentKey
                  ...
                  style={{
                    ...
                    borderTopRightRadius:
                      parentKey === scheme_data.length - 1 ? '4px' : 'none',
                    marginTop:
                      current_scheme_id === id && parentKey !== 0
                        ? '-2px'
                        : '0',
                ...
                {field_value.map((childValue, childKey) => {
                  ...
                  return (
                    <div
                      className={indexStyles.nounDefinit_col_item}
                      key={childKey}
                      style={{
                        borderLeft:
                          parentKey === 0 ? '1px solid #e5e5e5' : 'none',
                        borderBottom:
                          childKey === field_value.length - 1
                            ? '1px solid #e5e5e5'
                            : 'none',
                        borderBottomLeftRadius:
                          childKey === field_value.length - 1 && parentKey === 0
                            ? '4px'
                            : 'none',
                        borderBottomRightRadius:
                          childKey === field_value.length - 1 &&
                          parentKey === scheme_data.length - 1
                            ? '4px'
                            : 'none'
                      }}
                    >
                      {editable === '1' ? (
                        <input
                          ...
                          defaultValue={field_value_name}
                          placeholder={'输入自定义名词'}
                          maxLength={10}
                          ref={`Input_${field_data['field_value'][childKey]['id']}`}
                        />
                      ) : (
        ...
        {checkIsHasPermission(ORG_UPMS_ORGANIZATION_EDIT) ? (
          <div className={indexStyles.contain2}>
            <Button
              ...
              type={'primary'}
              onClick={this.saveNoun.bind(this)}
            >
              保存
            </Button>
          </div>
