...
import {
  ...
  workbench_selectFilePreviewCurrentVersionList,
  workbench_selectrUploadedFileList,
  workbench_selectBreadcrumbList,
  workbench_selectFilePreviewCurrentPreviewFileName,
  workbench_selectFilePreviewIsEntryCirclePreviewLoading,
  workbench_selectCurrentPreviewFileData
} from './selects'
//状态说明：
    ...
    setup({ dispatch, history }) {
      ...
      history.listen(location => {
        if (
          ...
          location.pathname === '/technological/simplemode'
        ) {
          const initialData = () => {
            dispatch({
              type: 'updateDatas',
  ...
  effects: {
    ...
    *fileVersionist({ payload }, { select, call, put }) {
      ...
      if (isApiResponseOk(res)) {
        if (new_breadcrumbList.length) {
          new_breadcrumbList[new_breadcrumbList.length - 1] =
    ...
    *fileConvertPdfAlsoUpdateVersion({ payload }, { select, call, put }) {
      ...
      let fileName = getSubfixName(
        yield select(workbench_selectFilePreviewCurrentPreviewFileName)
      )
