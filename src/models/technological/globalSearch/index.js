import {
  getGlobalSearchConditions,
  getFixedConditions,
  getGlobalSearchTypeList,
  getGlobalSearchResultList
} from '../../../services/technological'
import {
  selectSearchTypeList,
  selectDefaultSearchTypeNormal,
  selectSelectedConditions,
  selectDefaultSearchType,
  selectAllTypeResultList,
  selectPageNumber,
  selectPageSize,
  selectSigleTypeResultList,
  ...
  effects: {
    ...
    *getGlobalSearchTypeList({ payload = {} }, { call, put, select }) {
      ...
      if (isApiResponseOk(res)) {
        const de =
          res.data && typeof res.data[0] == 'object'
            ? res.data[0]['search_type']
            : ''
        yield put({
          type: 'updateDatas',
          payload: {
    ...
    *getMatchConditions({ payload = {} }, { call, put, select }) {
      ...
      const res = yield call(getGlobalSearchConditions, {
    ...
    *initDatas({ payload }, { select, call, put }) {
      const defaultSearchTypeNormal = yield select(
        selectDefaultSearchTypeNormal
      )
      yield put({
        type: 'updateDatas',
