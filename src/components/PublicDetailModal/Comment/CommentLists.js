...
export default class CommentListItem extends React.Component {
  ...
  getCommentList = props => {
    ...
    if (
      !content_detail_use_id ||
      content_detail_use_id == content_detail_use_id_local
    ) {
      return
    }
    this.setState({
        ...
        messageContainer = (
          <div className={commonCommentStyles.news_item}>
            <span className={commonCommentStyles.news_text}>
              <span className={commonCommentStyles.news_creator}>
                {data.creator && data.creator.name}
              </span>
              <span>创建了一条</span>
              <span>
                {data.content &&
                data.content.card_type &&
                data.content.card_type == '0'
                  ? currentNounPlanFilterName(TASKS)
                  : '会议'}
              </span>
              <span className={commonCommentStyles.news_card_name}>
                {data.content &&
                  data.content.card &&
                  data.content.card.name &&
                  data.content.card.name}
              </span>
            </span>
            <span>
        ...
        messageContainer = (
          <div className={commonCommentStyles.news_item}>
            <span className={commonCommentStyles.news_text}>
              ...
              <span className={commonCommentStyles.news_card_name}>
                {data.content &&
                  data.content.rela_data &&
                  data.content.rela_data.name &&
                  data.content.rela_data.name}
              </span>
        ...
        messageContainer = (
          <div className={commonCommentStyles.news_item}>
            <span className={commonCommentStyles.news_text}>
              ...
              <span className={commonCommentStyles.news_card_name}>
                {data.content &&
                  data.content.rela_card &&
                  data.content.rela_card.name &&
                  data.content.rela_card.name}
              </span>
              <span>中添加了一条</span>
  ...
  // 评论动态
  commentNews = data => {
    const { id: local_user_id } = localStorage.getItem('userInfo')
        ...
        messageContainer = (
          <div className={commonCommentStyles.common_item}>
            ...
            <div className={commonCommentStyles.common_left}>
              <Avatar
                src={data.creator && data.creator.avatar && data.creator.avatar}
                icon="user"
                style={{ color: '#8c8c8c' }}
              ></Avatar>
            </div>
            ...
            <div className={commonCommentStyles.common_right}>
              <div className={commonCommentStyles.common_top}>
                ...
                (local_user_id != (data.creator && data.creator.id) &&
                  data.creator.id) ? (
                  <div className={commonCommentStyles.common_create_time}>
                    {newsDynamicHandleTime(create_time)}{' '}
                    {timestampToHM(create_time)}
                  </div>
                ) : (
                  <div
                    className={commonCommentStyles.common_delete}
            ...
            <div className={commonCommentStyles.common_right}>
              <div className={commonCommentStyles.common_top}>
                ...
                {judgeTimeDiffer_ten(create_time) ? (
                  ...
                  >
                    {data.creator.id ==
                      JSON.parse(localStorage.getItem('userInfo')).id && '撤回'}
                  </div>
                )}
              </div>
              <div className={commonCommentStyles.common_bott}>
