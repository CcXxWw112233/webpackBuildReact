...
export default class OrgSearchAndSelect extends React.Component{
  ...
  setSelectKey = (e, type) => { //type add/remove
    ...
    const org_name = (currentUserOrganizes.find(item => item.id == key) || {}).name
    this.props.orgSelectedChange && this.props.orgSelectedChange({org_id: key, org_name})
  }
    ...
    return (
...

        {
          resultArr.map((value) => {
            const { name, id } = value
            return (
             <Menu.Item className={indexStyles.menuItem} style={{height: 32, lineHeight: '32px', margin: 0, padding: '0 10px', }} key={id} >
              {name}
             </Menu.Item>
            )
          })
